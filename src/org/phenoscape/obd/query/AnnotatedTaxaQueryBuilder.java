...
public class AnnotatedTaxaQueryBuilder extends QueryBuilder {
    ...
    public AnnotatedTaxaQueryBuilder(TaxonAnnotationsQueryConfig config, boolean totalOnly) {
        ...
        this.selectFields = String.format("%s.taxon_node_id, %s.taxon_uid, %s.taxon_label, %s.taxon_rank_node_id, %s.taxon_rank_uid, %s.taxon_rank_label, %s.taxon_is_extinct", this.table, this.table, this.table, this.table, this.table, this.table, this.table);
    }
    ...
    protected String getQuery() {
        ...
        if (intersects.isEmpty()) {
            baseQuery = String.format("SELECT DISTINCT %s FROM %s ", this.selectFields, this.table);    
        } else {
    ...
    private String getTaxonQuery(String taxonID) {
        final StringBuffer query = new StringBuffer();
        query.append(String.format("(SELECT DISTINCT %s FROM %s ", this.selectFields, this.table));
