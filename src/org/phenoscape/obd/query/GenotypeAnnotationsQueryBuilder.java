...
public class GenotypeAnnotationsQueryBuilder extends QueryBuilder {
    ...
    protected String getQuery() {
        ...
        if (intersects.isEmpty()) {
            baseQuery = "SELECT DISTINCT queryable_gene_annotation.* FROM queryable_gene_annotation ";    
        } else {
    ...
    private String getGenesQuery(List<String> genes) {
        ...
        query.append("(SELECT DISTINCT queryable_gene_annotation.* FROM queryable_gene_annotation WHERE ");
        query.append("queryable_gene_annotation.gene_uid IN ");
    ...
    private String getPhenotypeQuery(PhenotypeSpec phenotype) {
        final StringBuffer query = new StringBuffer();
        query.append("(SELECT DISTINCT queryable_gene_annotation.* FROM queryable_gene_annotation ");
        if (phenotype.getEntityID() != null) {
            if (phenotype.includeEntityParts()) {
                query.append(String.format("JOIN link phenotype_inheres_in_part_of ON (phenotype_inheres_in_part_of.node_id = queryable_gene_annotation.phenotype_node_id AND phenotype_inheres_in_part_of.predicate_id = %s) ", this.node(OBO.INHERES_IN_PART_OF)));    
            } else {
                query.append(String.format("JOIN link phenotype_inheres_in ON (phenotype_inheres_in.node_id = queryable_gene_annotation.phenotype_node_id AND phenotype_inheres_in.predicate_id = %s) ", this.node(OBO.INHERES_IN)));
            }
        }
        if (phenotype.getQualityID() != null) {
            query.append(String.format("JOIN link quality_is_a ON (quality_is_a.node_id = queryable_gene_annotation.quality_node_id AND quality_is_a.predicate_id = %s) ", this.node(OBO.IS_A)));
        }
        if (phenotype.getRelatedEntityID() != null) {
            query.append(String.format("JOIN link related_entity_is_a ON (related_entity_is_a.node_id = queryable_gene_annotation.related_entity_node_id AND related_entity_is_a.predicate_id = %s) ", this.node(OBO.IS_A)));  
        }
        query.append("WHERE ");
